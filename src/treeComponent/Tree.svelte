		...
		{#each treeData as treeNode}
			<li class:px-4="{visible}">
				{#if treeNode.children}
					<!-- show folder -->
					{#if treeNode.children}
						<svelte:self title={treeNode.title} treeData={treeNode.children} expanded={treeNode.expanded} treeConfig={treeConfig} disabled={treeNode.disabled} on:treeNodeDoubleClick={treeNodeDoubleClick}/>
					{:else}
						<TreeLeaf title={treeNode.title} visible={treeNode.visible} iconClass={treeConfig.icons.document} on:treeLeafDoubleClick={treeNodeDoubleClick}/>
					{/if}
				{:else}
					<TreeLeaf title={treeNode.title} visible={treeNode.visible} iconClass={treeConfig.icons.document} on:treeLeafDoubleClick={treeNodeDoubleClick}/>
				{/if}
